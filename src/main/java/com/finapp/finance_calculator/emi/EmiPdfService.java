...
public class EmiPdfService {
	...
	public static byte[] generateEmiPdf(EMIRequestDTO request, EMIResponseDTO response) {
		...
		try {
			...
			addCard(summary, "Total Payable (Principal + Interest)", format(response.getTotalAmount()), labelFont,
					valueFont);
			addCard(summary, "Interest Payable", format(response.getTotalInterest()), labelFont, valueFont);
			addCard(summary, "Principal Amount", format(request.getPrincipal()), labelFont, valueFont);

			addCard(summary, "EMI", format(response.getEmi()), labelFont, valueFont);
			int totalMonths = request.getDurationValue();
			int years = totalMonths / 12;
			int months = totalMonths % 12;
			addCard(summary, "Tenure", years + " Years " + months + " Months", labelFont, valueFont);
			addCard(summary, "Interest Rate", request.getAnnualRate() + " %", labelFont, valueFont);
	...
	private static void addCard(PdfPTable table, String label, String value, Font labelFont, Font valueFont) {
