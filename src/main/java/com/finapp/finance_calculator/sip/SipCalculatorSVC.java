...
public class SipCalculatorSVC {
	...
	public SipResponseDTO calculateSip(SipRequestDTO request) {
		...
		int durationYears = request.getDurationYears();

		double monthlyRate = annualRate / 12 / 100;
		int totalMonths = durationYears * 12;

		double maturityValue = monthlyInvestment * (Math.pow(1 + monthlyRate, totalMonths) - 1) * (1 + monthlyRate)
				/ monthlyRate;
		...
		for (int year = 1; year <= durationYears; year++) {
			int months = year * 12;
			double value = monthlyInvestment * (Math.pow(1 + monthlyRate, months) - 1) * (1 + monthlyRate)
					/ monthlyRate;
			double invested = monthlyInvestment * months;
			double interest = value - invested;

			breakdown.add(new SIPYearlyBreakDownDTO(year, round(invested), round(interest), round(value)));
		}

		return new SipResponseDTO(round(totalInvested), round(estimatedReturns), round(maturityValue), breakdown);
	}
